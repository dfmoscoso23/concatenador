...
dic_imagenes = {}
imagenes_cambiadas=[]
dic_error_subida_imagenes={}
dic_error_catalogo_imagenes={}
global sa
sa=0
global sl
sl=50
...
def diccionador(libro2, hoja2, columna_isbn, columa_precio, barra_actualizar):
	...
	if revisor_desdeExcel == True:
		...
		"""
		matchear_ml_isbn()
		for item, isbn in dic_isbn.items():
...
def cambiar_imagenes(isbn):
	lista_para_subir = []
	for i in dic_imagenes[isbn]:
		lista_para_subir.append({"source":i})
	try:	
		print(dic_ml[str(isbn)])
		url="https://api.mercadolibre.com/items/"+ str(dic_ml[str(isbn)])
		headers = {"Authorization": str("Bearer "+tok),"Content-Type": "application/json","Accept": "application/json"}
		data= {"pictures":lista_para_subir}
		cambio = requests.put(url, headers=headers, json=data)
		print(cambio.status_code)
		if cambio.status_code==200:
			imagenes_cambiadas.append(isbn)
		else:
			dic_error_subida_imagenes[isbn]=cambio.content
	except KeyError:
		dic_error_catalogo_imagenes[isbn]="No está en la base"			
def deconstruirisbns(presibn, listas):
	imag = []
	if presibn[-7] != "0":
		messagebox.showinfo('Aviso', presibn + ' no pudo ser concatenado por no tener el formato adecuado')
		...
		if presibn[-21] == "/":
			...
			if resto == "001.jpg":
				...
				imag.append(listas[0])
				listas.pop(0)
def armador_listas_imagenes(isbn):
	imag=[]
	if isbn in portada:
		imag.append(portada[isbn])
	if isbn in contraportada:
		imag.append(contraportada[isbn])
	if isbn in paginaextra1:
		imag.append(paginaextra1[isbn])
	if isbn in paginaextra2:
		imag.append(paginaextra2[isbn])
	if isbn in paginaextra3:
		imag.append(paginaextra3[isbn])
	if isbn in paginaextra4:
		imag.append(paginaextra4[isbn])				
	imag.append("https://i.postimg.cc/B6SMfSSh/001.jpg")		
	dic_imagenes[isbn]=imag		
def imaginador():
	global ingreso_imag
	lista = ingreso_imag.get()
	listas = lista.split()
	preisbns = listas[:]
	matchear_ml_isbn()
	while len(listas)>0:
	...
	for isbn in isbns:
		armador_listas_imagenes(isbn)
		cambiar_imagenes(isbn)
	imag_camb=len(imagenes_cambiadas)			
	messagebox.showinfo("Proceso Concluido","Se cambiaron "+str(imag_camb)+" publicaciones")
	imag_ncamb=len(dic_error_subida_imagenes)
	messagebox.showerror("Error", "No se pudo actualizar "+str(imag_ncamb)+" publicaciones")
def errores_imagenes():
	nv = Toplevel(raiz)
	cuadro_excluidos = Text(nv, width=75, height=25)
	cuadro_excluidos.pack()
	for i,e in dic_error_subida_imagenes.items():
		err=e.decode()	
		cuadro_excluidos.insert(END,i+">>>>>>>>ERROR>>>>>>"+err+"\n")
	for i,e in dic_error_catalogo_imagenes.items():
		err=e	
		cuadro_excluidos.insert(END,i+">>>>>>>>ERROR>>>>>>"+err+"\n")
	cuadro_excluidos.insert(END,dic_ml)	
def imagenes_cambiadas_desplegable():
	nv = Toplevel(raiz)
	cuadro_excluidos = Text(nv, width=75, height=25)
	cuadro_excluidos.pack()
	for i in imagenes_cambiadas:
		cuadro_excluidos.insert(END,i+"\n")
def desplegar_imaginador():
	if revisor_desdeExcel!= True:
		messagebox.showerror("Requiere acción", "Falta cargar el catálogo desde Excel")	
	else:	
		imaginador_desplegable = Toplevel(raiz)
		imaginador_desplegable.title("Corrección de imágines - Losada")
		frame_sup = Frame(imaginador_desplegable)
		frame_sup.pack()
		ingrese = Label(frame_sup, text="Ingrese URL de imágenes:")
		ingrese.grid(column=0, row=1)
		global ingreso_imag
		ingreso_imag = Entry(frame_sup, width=75)
		ingreso_imag.grid(column=1, row=1, padx=5, pady=5)
		boton_imaginador = Button(frame_sup, text="Cambiar imágenes", command=imaginador)
		boton_imaginador.grid(column=2, row=1, padx=5, pady=5)
		frame_bajo= Frame(imaginador_desplegable)
		frame_bajo.pack()
		boton_desplegar_errores_imagenes = Button(frame_bajo, text="Errores de publicación", command=errores_imagenes)
		boton_desplegar_errores_imagenes.grid(column=0, row=0, padx=5, pady=5)
		boton_desplegar_imagenes_cambiados = Button(frame_bajo, text="Publicaciones actualizadas", command=imagenes_cambiadas_desplegable)
		boton_desplegar_imagenes_cambiados.grid(column=1, row=0, padx=5, pady=5)
...
#Frame2 - Botones
bot_obtener_token = Button(frame2, text="Obtener Token", command=revisar_tokenpresxistente)
...
bot_imaginador = Button(frame2, text="Actualizar imágenes", command=desplegar_imaginador)
bot_imaginador.grid(column=1, row=4, padx=5, pady=5)
